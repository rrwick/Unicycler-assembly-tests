...
filter_results <- function(results, exclude_random_seqs = TRUE, read_set_type = NULL, assembler = NULL, version = NULL, setting = NULL, illumina_qual = NULL) {
...
short_read_table <- function(results, illumina_qual) {
  result_groups = list()
  result_groups[[1]] <- filter_results(results, assembler = "Velvet", version = "1.2.10", illumina_qual = illumina_qual, exclude_random_seqs = TRUE, read_set_type = "short-only")
  ...
  short_read_results <- data.frame(assembler_names, assembler_versions, assembler_settings, n50, nga50, misassemblies, small_err, time)  
  colnames(short_read_results) = c("Assembler", "Version", "Setting/output", "N50", "NGA50", "Misassemblies", "Small errors (per 100 kbp)", "Time (minutes)")
  
  kable(short_read_results, format = "markdown")
}
